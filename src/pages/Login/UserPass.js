    ...
    const getBearerTokenFromLogin = async (userInvalid, passInvalid) => {
        if (userInvalid === false && passInvalid === false) {
            ...
            if (response) {
                ...
                await dispatch({ type: 'SET_DEVICES_TO_REGISTER', payload: unregisteredDevices(decodedToken.roles) });
                await dispatch({ type: 'SET_STARTED_GARAGE_REGISTRATION', payload: Boolean(garageRole && garageRole.device_id) });
